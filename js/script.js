...
function initializeTutorials() {
    ...
    document.querySelectorAll('.tutorial-overlay').forEach(overlay => {
        overlay.addEventListener('click', (e) => {
            const tutorial = e.target.closest('.tutorial-card').dataset.tutorial;
            startTutorial(tutorial);
        });
    });
...
function startTutorial(tutorialType) {
    console.log(`📚 Iniciando tutorial: ${tutorialType}`);
    
    // Actualizar progreso del tutorial
    updateTutorialProgress(tutorialType, 10);
    
    // Mostrar modal o redirigir al tutorial específico
    const tutorialTitles = {
        'cpu-architecture': 'Arquitectura del CPU',
        'memory-ram': 'Memoria RAM G.Skill Trident Z5',
        'mouse-logitech': 'Mouse Logitech MX Master 3S',
        'memory-hierarchy': 'Jerarquía de Memoria',
        'data-flow': 'Flujo de Datos en Sistemas Embebidos',
        'motherboard': 'Arquitectura de Placa Base'
    };
    
    const title = tutorialTitles[tutorialType] || 'Tutorial';
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>¿Estás listo para comenzar este tutorial interactivo?</p>
                <div class="tutorial-info">
                    <span><i class="fas fa-clock"></i> Duración estimada</span>
                    <span><i class="fas fa-star"></i> Nivel de dificultad</span>
                    <span><i class="fas fa-users"></i> Estudiantes inscritos</span>
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn-secondary modal-cancel">Cancelar</button>
                <button class="btn-primary modal-start">Comenzar Tutorial</button>
            </div>
        </div>
    `;
    ...
    modal.querySelector('.modal-start').addEventListener('click', () => {
        ...
        console.log(`🚀 Tutorial ${tutorialType} iniciado`);
    });
...
function filterTutorials(filter) {
    console.log(`🔍 Filtrando tutoriales por: ${filter}`);
    ...
    // Actualizar contador de tutoriales visibles
    updateTutorialCount(filter);
}

// Actualizar contador de tutoriales
function updateTutorialCount(filter) {
    const visibleCards = document.querySelectorAll('.tutorial-card[style*="block"], .tutorial-card:not([style*="none"])');
    const count = filter === 'all' ? visibleCards.length : 
                  Array.from(visibleCards).filter(card => card.dataset.category === filter).length;
    
    console.log(`📊 Mostrando ${count} tutoriales de la categoría: ${filter}`);
}

// Actualizar progreso del tutorial
function updateTutorialProgress(tutorialType, progress) {
    const card = document.querySelector(`[data-tutorial="${tutorialType}"]`);
    if (card) {
...
// Simular progreso de tutoriales (para demostración)
function simulateTutorialProgress() {
    const tutorials = ['cpu-architecture', 'memory-ram', 'mouse-logitech', 'memory-hierarchy', 'data-flow'];
    
    tutorials.forEach((tutorial, index) => {
        setTimeout(() => {
            const randomProgress = Math.floor(Math.random() * 100);
            updateTutorialProgress(tutorial, randomProgress);
        }, index * 1000);
    });
}

// Inicializar Asistente IA (solo para compatibilidad, la lógica real está en app.js)
function initializeAIAssistant() {
    console.log('🤖 Asistente IA inicializado - usando funciones de app.js');
    // La lógica del asistente IA está en app.js
}
